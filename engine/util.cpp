...
int DrawText(std::string text, std::vector<float> &arr, float x, float y)
{
	...
	while (i < text.size() && i < arr.size()/24){
			...
			arr[i*24 + 0] = 0+x+(i*8);
			arr[i*24 + 1] = y;
			arr[i*24 + 2] = magicxoffset*((character%32));
			arr[i*24 + 3] = 1-(magicyoffset*(character/32));

			//BR
			arr[i*24 + 4] = 8+x+(i*8);
			arr[i*24 + 5] = y;
			arr[i*24 + 6] = magicxoffset*((character%32)+1);
			arr[i*24 + 7] = 1-(magicyoffset*(character/32));

			//TR
			arr[i*24 + 8] = 8+x+(i*8);
			arr[i*24 + 9] = y-12;
			arr[i*24 + 10] = magicxoffset*((character%32)+1);
			arr[i*24 + 11] = 1-(magicyoffset*((character/32)+1));

			//SPLIT
			//TR
			arr[i*24 + 12] = 8+x+(i*8);
			arr[i*24 + 13] = y-12;
			arr[i*24 + 14] = magicxoffset*((character%32)+1);
			arr[i*24 + 15] = 1-(magicyoffset*((character/32)+1));

			//TL
			arr[i*24 + 16] = 0+x+(i*8);
			arr[i*24 + 17] = y-12;
			arr[i*24 + 18] = magicxoffset*((character%32));
			arr[i*24 + 19] = 1-(magicyoffset*((character/32)+1));

			//TL
			arr[i*24 + 20] = 0+x+(i*8);
			arr[i*24 + 21] = y;
			arr[i*24 + 22] = magicxoffset*((character%32));
			arr[i*24 + 23] = 1-(magicyoffset*(character/32));
